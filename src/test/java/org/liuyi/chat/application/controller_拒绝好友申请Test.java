...
public class controller_拒绝好友申请Test {
    ...
    @Test
    void 拒绝好友申请时_如果参数正常_响应应该符合预期_事件应该被正确发送() {
        String operatorId = commonFriendApplication.getTargetUserId();
        String applicationId = commonFriendApplication.getId();
        Instant operateTime = Instant.now();
        RejectFriendApplication200Response resp = application.rejectFriendApplication(operatorId, applicationId, operateTime);
    ...
    void 拒绝好友申请时_如果好友申请不存在_错误码应该正确设置() {
        ...
        RejectFriendApplication200Response resp = application.rejectFriendApplication(operatorId, applicationId, Instant.now());
    ...
    void 拒绝好友申请时_如果操作人不是好友申请的目标用户_错误码应该正确设置() {
        String operatorId = commonFriendApplication.getApplicantUserId(); // 申请用户自己不能处理好友申请
        String applicationId = commonFriendApplication.getId();
        RejectFriendApplication200Response resp = application.rejectFriendApplication(operatorId, applicationId, Instant.now());
        assertFalse(resp.getSuccess());
    }
